...
export async function generateMetadata(props) {
    ...
    if (!productId || !Number.isFinite(productId) || productId <= 0) {
        return {
            title: 'Product Not Found | Apple Face BD BD',
        };
    }
    ...
    try {
        ...
        const title = `${product.name} - Apple Face BD BD`;
        const description = product.description
            ? product.description.replace(/<[^>]+>/g, '').substring(0, 150) + '...'
            : `Buy ${product.name} at the best price in Bangladesh from Apple Face BD BD.`;
        ...
        return {
            title: 'Product Details | Apple Face BD BD',
        };
    }
}
